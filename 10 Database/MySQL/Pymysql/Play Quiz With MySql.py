...
def admin_operation():
    ...
    try:
        con = mysql.connector.connect(host="localhost",user="root",
                                      password="mysql",database="quiz")
        cursor=con.cursor()
        cursor.execute('''CREATE TABLE science(SrNo INT(5),Question TEXT,
    ...
    if ch==1:
        ...
        cursor = con.cursor()
        cursor.execute(f"INSERT INTO science VALUES({qno},'{ques}','{opt1}','{opt2}','{opt3}','{opt4}',{ans})")
        #cursor.execute("INSERT INTO science VALUES("+qno+",'"+ques+"','"+opt1+"','"+opt2+"','"+opt3+"','"+opt4+"','"+ans+"')")
        con.commit()
        con.close()
    elif ch==2:
        qno = int(input("Enter the question no.: "))
        cursor.execute(f"SELECT * FROM science WHERE SrNo={qno}")
        for i in cursor:
        ...
        con = mysql.connector.connect(host="localhost",user="root",
                                      password="mysql",database="quiz")
        cursor = con.cursor()
        cursor.execute(f"UPDATE science SET Question ='{que}' WHERE SrNo={qno}")
        con.commit()
        con.close()
    elif ch==3:
        qno = int(input("Enter the Question No."))
        ...
        con = mysql.connector.connect(host="localhost",user="root",
                                      password="mysql",database="quiz")
        cursor = con.cursor()
        cursor.execute(f"UPDATE science SET Answer ={ans} WHERE SrNo={qno}")
        con.commit()
        con.close()
    elif ch==4:
        con = mysql.connector.connect(host="localhost",user="root",
        ...
        cursor.execute(f"UPDATE science SET Option{ano} ='{optn}' WHERE SrNo={qno}")
        con.commit()
        con.close()    
    else:
...
def play_Quiz():
    con = mysql.connector.connect(host="localhost",user="root",
                                      password="mysql",database="quiz")
    cursor = con.cursor()
    cursor.execute(f"SELECT * from Science")
    for i in cursor:
        print(f"SrNo{i[0]}: {i[1]} \nOption1:{i[2]} \nOption2:{i[3]} \nOption3:{i[4]} \nOption4:{i[5]}")
